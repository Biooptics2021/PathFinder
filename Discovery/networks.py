...
# from batch_renormalization import BatchRenorm1d as br
...
def regularize_path_weights(model, reg_type=None):
    ...
    for W in model.module.fc.parameters():
        if l1_reg is None:
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        # hazard = self.bn3(hazard)
        # hazard = self.sigmoid(hazard)
        # hazard = self.tanh(hazard)
        # hazard = hazard*hazard
        # hazard = self.relu(hazard)
        

        # return features, hazard 
        return hazard#Can return features
